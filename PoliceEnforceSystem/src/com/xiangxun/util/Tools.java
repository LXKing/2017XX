...
import com.xiangxun.wheelview.NumericWheelAdapter;
import com.xiangxun.wheelview.OnWheelChangedListener;
import com.xiangxun.wheelview.WheelView;
...
public class Tools {
	...
	public static boolean isIDCardValidate(String IDStr) {  
        ...
        if (IDStr.length() != 15 && IDStr.length() != 18) {
        	MsgToast.geToast().setMsg("驾驶证号码长度应该为15位或18位。");  
            return false;  
        }  
        // =======================(end)========================  
        ...
        if (isNumeric(Ai) == false) {  
            MsgToast.geToast().setMsg("驾驶证15位号码都应为数字 ; 18位号码除最后一位外，都应为数字。");  
            return false;  
        }  
        // =======================(end)========================  
        ...
        if (isDataFormat(strYear + "-" + strMonth + "-" + strDay) == false) {  
        	MsgToast.geToast().setMsg("驾驶证号中生日无效。");  
            return false;  
        }  
        ...
        try {  
            if ((gc.get(Calendar.YEAR) - Integer.parseInt(strYear)) > 150  
            	...
            	MsgToast.geToast().setMsg("驾驶证号中生日不在有效范围。");  
                return false;  
            }  
        ...
        if (Integer.parseInt(strMonth) > 12 || Integer.parseInt(strMonth) == 0) {  
        	MsgToast.geToast().setMsg("驾驶证号中月份无效");  
            return false;  
        }  
        if (Integer.parseInt(strDay) > 31 || Integer.parseInt(strDay) == 0) {  
        	MsgToast.geToast().setMsg("驾驶证号中日期无效");  
            return false;  
        }  
        // =====================(end)=====================  
        ...
        if (h.get(Ai.substring(0, 2)) == null) {  
        	MsgToast.geToast().setMsg("驾驶证号码地区编码错误。");  
            return false;  
        }  
        // ==============================================  
        ...
        if (IDStr.length() == 18) {  
            if (Ai.equals(IDStr) == false) {  
            	MsgToast.geToast().setMsg("驾驶证无效，不是合法的驾驶证号码");  
                return false;  
            }  
        } else {  
            return true;  
	...
	public static boolean isCarnumberNO(String carnumber, String carplatetype) {
		/*
		 * 车牌号格式：汉字 + A-Z + 5位A-Z或0-9 （只包括了普通车牌号，教练车和部分部队车等车牌号不包括在内）
		 */
		String carnumRegex = "[\u4e00-\u9fa5]{1}[A-Z]{1}([A-Z_0-9]{5}|[A-Z_0-9]{4}[\u4e00-\u9fa5]{1})";
		String carnumRegexEx = "[\u4e00-\u9fa5]{1}[A-Z]{1}([A-Z_0-9]{5}|[A-Z_0-9]{4})";
		if (TextUtils.isEmpty(carnumber))
			return false;
		else {
			if (carplatetype.contains("警") || carplatetype.contains("教") || carplatetype.contains("挂") || carplatetype.contains("馆"))
				return carnumber.matches(carnumRegexEx);
			else
