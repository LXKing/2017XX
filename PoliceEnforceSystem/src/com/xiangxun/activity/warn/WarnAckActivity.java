...
public class WarnAckActivity extends BaseActivity implements OnClickListener, selectVioTypeOnClick, DeletePictureInterface, OnPageChangeListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		SignAndAck = (boolean)intent.getBooleanExtra("signandack", false);
		// 检测是否预警签收浏览
		if (1 == warnData.getIsAck()) {
			warnAckView = true;
			//获取预警反馈信息（非签收） 业务类型 1-签收；	2-反馈；3-签收反馈
			WarnMessage dt = DBManager.getInstance().GetWarnMessages(warnData.getYjxh());
			warnAck = dt.getWarnAck();
			warnPic = dt.getWarnPic();
		}
...
//		<string name="wran_note">预警序号: %1$s\n预警时间: %2$s\n卡口名称: %3$s\n方向类型: %4$s\n号牌种类: %5$s\n号牌号码: %6$s\n布控类型: %7$s</string>
	...
	private boolean savePic() {
		...
		picInfo.scdw = SystemCfg.getDepartmentCode(this);	//处置单位
		picInfo.scr = SystemCfg.getUpdateName(this); //edtPolice.getText().toString();	//处置民警
...
//		}
		DBManager.getInstance().WarnMessageUpdate(warnData.getYjxh(), "pic", XiangXunApplication.getGson().toJson(picInfo));
	...
	private boolean saveDB() {
		...
		ackInfo.czr = SystemCfg.getUpdateName(this);//edtPolice.getText().toString();	//处置民警
...
//		DBManager.getInstance().add(ackInfo);
		DBManager.getInstance().WarnMessageUpdate(warnData.getYjxh(), "ack", XiangXunApplication.getGson().toJson(ackInfo));
		warnData.setIsOk(1);
		warnData.setIsDo(1);
		warnData.setIsAck(1);
		DBManager.getInstance().WarnMessageUpdate(warnData.getYjxh(), "data", XiangXunApplication.getGson().toJson(warnData));
		DBManager.getInstance().WarnMessageUpdate(warnData.getYjxh(), "step", "3");
	...
	private void GoToWarnTable(String doType) {
		if (false == SystemCfg.getIsPrint(this)) {
			...
			Intent intent = new Intent();
			intent.putExtra("yjxh", warnData.getYjxh());
			intent.putExtra("hphm", warnData.getHphm());
...
//			if (!vioType.equals("简易程序处罚决定书")) {
//				MsgToast.geToast().setMsg("请确保文书已上传六合一!!!");
//			}
