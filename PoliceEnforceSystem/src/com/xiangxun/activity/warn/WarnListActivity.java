...
public class WarnListActivity extends BaseActivity implements IXListViewListener, SignAndAckOnClick {
	...
	private void addWarnData(List<WarnMessage> datas) {
		for (int i = 0, len = datas.size(); i < len; i++) {
			WarnMessage warnMessage = datas.get(i);
			if (warnMessage.dataInfo != null) {
				warnDatas.add(warnMessage.getWarnData());
			}
		}
	}
	...
	public void initListener() {
		...
		mXListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View view, int index, long arg3) {
				...
				intent.putExtra("warndata", (Serializable)data);
				if (-1 == data.getIsOk()) {
					// 签收
	...
	public void initData() {
		...
		if (DBManager.getInstance().QueryWarnMessageCount() <= 0) {
			mVF.setDisplayedChild(2);
		} else {
			RequestList();
		}
...
//				mAdapter.setData(warnDatas);
	...
	private void RequestList() {
		...
		DBManager.getInstance().GetWarnMessages(currentPage, PageSize, mUIHandler);
	}
