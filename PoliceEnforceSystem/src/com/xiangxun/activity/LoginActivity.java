...
//		mTakePic = (ImageView) this.findViewById(R.id.take_pic);
	...
	public void initData() {
		...
		mBtnLogin.setViewInit(R.string.mine_login_login, R.string.mine_login_loginning, mUsername, mPassword);	//, mSettings, mTakePic);
		...
		rememberCb.setChecked(SystemCfg.getIsSavePassword(this));
...
//		mTakePic.setOnClickListener(this);
	...
	private void setUsernameAndPassword() {
		...
		if (password.length() > 1 && SystemCfg.getIsSavePassword(this)) {
		...
		if (login != null) {
			...
			if (login.res.equals("no_name")) {
				...
				if (userInfos != null) {
					...
					String laderName = "";
					String laderCode = "";
					if (userInfos.puserCode.startsWith("b") || userInfos.puserCode.startsWith("B")) {
						if (userInfos.memo != null && userInfos.memo.length() > 6){
							laderCode = userInfos.memo.substring(0,6);
							laderName = userInfos.memo.substring(6);
						} else {
							MsgToast.geToast().setMsg("此警员未分配组长,请联系管理员绑定组长信息！");
						}
					} else {
						laderCode = userInfos.puserCode;
						laderName = userInfos.name;
					}

					SystemCfg.setUpdateName(this, laderName);
					SystemCfg.setUpdateCode(this, laderCode);
				}
...
//				ResultData.JiaoDaoYuan jiaoDaoYu = loginData.jiaodaoyuan;
//				if (jiaoDaoYu != null) {
//					SystemCfg.setUpdateName(this, null != jiaoDaoYu.name ? jiaoDaoYu.name.toString() : "");
//					SystemCfg.setUpdateCode(this, null != jiaoDaoYu.code ? jiaoDaoYu.code.toString() : "");
//				}
...
//			break;
		case R.id.login_btn:
			SystemCfg.setIsSavePassword(this, rememberCb.isChecked());
