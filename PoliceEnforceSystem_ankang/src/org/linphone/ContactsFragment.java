...
public class ContactsFragment extends Fragment implements OnClickListener, OnItemClickListener {
	...
	public void onClick(View v) {
		...
		if (id == R.id.allContacts) {
			...
			editConsumed = true;
			ContactActivity.instance().addContact(null, sipAddressToAdd);
	...
	private void changeContactsAdapter() {
		...
		Cursor allContactsCursor = ContactActivity.instance().getAllContactsCursor();
		Cursor sipContactsCursor = ContactActivity.instance().getSIPContactsCursor();
		...
		if (onlyDisplayLinphoneContacts) {
			if (sipContactsCursor.getCount() == 0) {
				...
				contactsList.setAdapter(new ContactsListAdapter(ContactActivity.instance().getSIPContacts(), sipContactsCursor));
			}
		} else {
			if (allContactsCursor.getCount() == 0) {
				...
				contactsList.setAdapter(new ContactsListAdapter(ContactActivity.instance().getAllContacts(), allContactsCursor));
			}
		}
		//ContactActivity.instance().setLinphoneContactsPrefered(onlyDisplayLinphoneContacts);
	}
	...
	public void onItemClick(AdapterView<?> adapter, View view, int position, long id) {
		...
		//extras.putBoolean("ChatAddressOnly", true);
...
//			ContactActivity.instance().selectMenu(FragmentsAvailable.CONTACTS);
//			onlyDisplayLinphoneContacts = ContactActivity.instance().isLinphoneContactsPrefered();
//
...
//		}
	...
	class ContactsListAdapter extends BaseAdapter implements SectionIndexer {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (!ContactActivity.instance().isContactPresenceDisabled() && friend != null) {
