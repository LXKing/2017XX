...
import com.xiangxun.activity.contact.AddContactActivity;
...
public class EditContactFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		ok.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				try {
					...
					ContactActivity.instance().prepareContactsInBackground();
				} catch (Exception e) {
					e.printStackTrace();
				}
				AddContactActivity.instance().onBackPressed();
			}
		});
		...
		if (Version.sdkStrictlyBelow(Version.API11_HONEYCOMB_30)) {
			lastName.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
					InputMethodManager imm = (InputMethodManager) ContactActivity.instance().getSystemService(Context.INPUT_METHOD_SERVICE);
	...
	private String findRawContactID(String contactID) {
		Cursor c = getActivity().getContentResolver().query(RawContacts.CONTENT_URI, new String[] { BaseColumns._ID }, RawContacts.CONTACT_ID + "=?", new String[] { contactID }, null);
		if (c != null) {
			String result = null;
			if (c.moveToFirst()) {
				result = c.getString(c.getColumnIndex(BaseColumns._ID));
			}
			c.close();
			return result;
		}
