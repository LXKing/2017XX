...
public class BluetoothPrintDriver {
	...
	public static void LocateBlackLabelMode() {
		byte[] cmd = new byte[]{(byte)12};
		BT_Write(cmd, cmd.length);
	}

	public static void LocateBlackLabel_1Byte() {
		byte[] cmd = new byte[]{(byte)12};
		BT_Write(cmd, 1);
	}

	public static void LocateBlackLabel_2Byte() {
		byte[] cmd = new byte[]{(byte)18, (byte)65};
		BT_Write(cmd, 2);
	}

	public static void PrintAndFeedLines(int n) {
		byte[] cmd = new byte[]{(byte)27, (byte)100, (byte)n};
		BT_Write(cmd, 3);
	}
	...
	private class AcceptThread extends Thread {
		...
		public void run() {
			...
			while(BluetoothPrintDriver.mState != STATE_CONNECTED) {
				try {
