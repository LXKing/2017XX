...
//			PrintAndFeedLines(1);
	...
	private void PrintBold(String str) {
		BluetoothPrintDriver.SetBold((byte)1);
		BluetoothPrintDriver.printString(str);
	}
	...
	private void SetAlign(int align) {
		BluetoothPrintDriver.SetAlignMode((byte)align);
	}

	private void PrintBlackLabel() {
		BluetoothPrintDriver.SetBlackReversePrint((byte)1);
		PrintNormal("   ");
		BluetoothPrintDriver.SetBlackReversePrint((byte)0);
	}

	private void SearchBlackLabel() {
		String printerName = SystemCfg.getPrinterName(XiangXunApplication.getInstance());
		if ("Feasycom".equals(printerName))
			BluetoothPrintDriver.LocateBlackLabel_2Byte();
		else	//瑞工
			BluetoothPrintDriver.LocateBlackLabel_1Byte();
	}
