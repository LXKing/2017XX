	...
	private class MQTTConnection implements MqttSimpleCallback {
		...
		public void publishArrived(String topicName, byte[] payload, int qos, boolean retained) {
			...
			if (!LinphoneService.isReady()) {
				startService(new Intent(ACTION_MAIN).setClass(MQTTService.this, LinphoneService.class));
			}
			...
			if (6 == sm.msgtype) {
				...
				String cross = SystemCfg.getWarnCross(MQTTService.this);
				Gson gson = new Gson();
				WarnData wranData = gson.fromJson(sm.text, WarnData.class);
				if (!cross.contains(wranData.getKkmc()))
					return;
				DBManager.getInstance().add(wranData);
		...
		public void sendKeepAlive() throws MqttException {
			...
			publishToTopic("xiangxun" + XiangXunApplication.getInstance().getDevId(), "");
		}
	}
