			...
			try {
				if (true) {
					...
					case ConstantStatus.UPLOAD_ALLDATA:
						...
						// 上传预警反馈信息
						int countWarnAck = DBManager.getInstance().QueryUnUpWarnAckDataCount();
						if (countWarnAck > 0) {
							dm.upWarnAckData();
							//	countWarnAck = DBManager.getInstance().QueryUnUpWarnAckDataCount();
						}
						int countWarnPic = DBManager.getInstance().QueryUnUpWarnPicDataCount();
						if (countWarnPic > 0) {
							dm.upWarnPicData();
							//	countWarnPic = DBManager.getInstance().QueryUnUpWarnPicDataCount();
						}
						break;
					}
				}
