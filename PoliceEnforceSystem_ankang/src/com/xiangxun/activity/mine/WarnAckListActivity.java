...
import com.xiangxun.activity.warn.WarnAckActivity;
...
public class WarnAckListActivity extends BaseActivity implements IXListViewListener {
	...
	private List<WarnAck> WarnAckInfos = new ArrayList<WarnAck>();
	...
	private Handler mUIHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case ConstantStatus.SREACH_DETAIL_SUCCESS:
				...
				List<WarnAck> datas = (List<WarnAck>) msg.obj;
				if (datas != null) {
	...
	public void initListener() {
		...
		mXListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View view, int index, long arg3) {
				...
				WarnAck data = (WarnAck) mAdapter.getItem(index - 1);
				WarnData warnData = DBManager.getInstance().getWarnDataInfo(data.yjxh);
	...
	public void initData() {
		...
		if (DBManager.getInstance().QueryWarnAckDataCount() <= 0) {
			mVF.setDisplayedChild(2);
		} else {
			RequestList();
		}
	...
	private void RequestList() {
		...
		DBManager.getInstance().getWarnAckInfos(currentPage, PageSize, mUIHandler);
	}
